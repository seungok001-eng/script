...
export interface Ranks {
  first?: number; // 1순위 항목 번호
  second?: number; // 2순위 항목 번호
  reasons: Record<number, string>; // 항목 번호 → 추천 이유
}

/**
 * '## 추천/몰입 추천' 섹션에서 1·2순위 항목 번호와 이유를 추출한다.
 *   1순위: <labelWord> 3 - 이유
 *   2순위: <labelWord> 1 - 이유
 * @param labelWord "주제" | "프로필" | "세트"
 */
export function parseRanks(text: string, labelWord: string): Ranks {
  const res: Ranks = { reasons: {} };
  if (!text?.trim()) return res;
  const lines = text.split("\n");
  const numRe = new RegExp(`${labelWord}\\s*(\\d+)`);
...
/** @deprecated parseRanks(text, "프로필") 사용 */
export type ProfileRanks = Ranks;
export const parseProfileRanks = (text: string): Ranks => parseRanks(text, "프로필");
export const parseTopicRanks = (text: string): Ranks => parseRanks(text, "주제");
export const parseSetRanks = (text: string): Ranks => parseRanks(text, "세트");
